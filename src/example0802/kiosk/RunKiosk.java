...
public class RunKiosk {
    ...
    public static void main(String[] args) throws CustomException {
        ...
        try {
            while (true) {
                System.out.println("메뉴 또는 주문 입력해주세요");
                String userMenu = sc.next();
                if (userMenu.equals("주문")) {
                    break;
                }
                userMenus.add(userMenu);
            }
            Menu[] menu = kiosk.setMenus(userMenus);

            System.out.println("1.배달 2.포장 3.매장 주문방식 번호를 입력해주세요");
            int userInput = sc.nextInt();

            Order orderService = kiosk.initOrder(OrderParameters.builder()
                    .menu(menu)
                    .type(userInput)
                    .build());
            ...
            if(orderService.runOrder(money)){
                ((DeliveryOrder)orderService).setLocate(address);
            }else{
                System.out.println("금액이 적음");
            }
            ((DeliveryOrder) orderService).successDelivery();
            kiosk.subInventory(userMenus.size());
